...
%\textbf{Senan note:} Beef up this description of the MTE IV caveats, with a details laden appendix section.
%Suppose $\vec X_i^{\text{IV}}$ takes $K$ distinct values, then identify MTE up to $K-1$ parameters.
%In addition, estimation requires separability of $U_i$ and $\vec X_i$.
%Also extrapolation from $\vec X_i^{\text{IV}}$ compliers to $D_i(z)$ compliers is more credible if it is internal:
%\[ \min \pi(z; \vec X_i) \leq \E{\pi(0; \vec X_i)} \leq \E{\pi(1; \vec X_i)} \leq \max \pi(z; \vec X_i). \]
