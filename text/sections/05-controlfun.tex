...
where $\bar D = \frac1N \sum_{i=1}^N D_i$, $\bar Z = \frac1N \sum_{i=1}^N Z_i$, and $\Gamma(.,.)$ is the mean estimate of the complier adjustment term as a function of $\lambda_1$.
...
$\hat\delta = (\hat{\gamma + \delta}) - \hat\gamma$ for the point estimate of $\gamma$, to give the following representation,\footnote{
...
\[ \hat{\text{AIE}}^{\text{CF}}
    = \hat{\text{ATE}}
    - (1 - \bar Z) \, \left(\hat\gamma + \hat \delta
        \,\frac{\sum_{i = 1}^N D_i Z_i}{\sum_{i = 1}^N Z_i} \right)
    - \bar Z \, \left(\hat\gamma + \hat \delta
        \,\frac{\sum_{i = 1}^N D_i (1 - Z_i)}{\sum_{i = 1}^N (1 - Z_i)} \right). \]
