...
Note that $\lambda_0$ can no longer be separated from $\rho_0$ in this semi-parametric approach; the complier adjustment term requires $\rho_0, \rho_1$ only be identified up to a constant.\footnote{
...
An efficient estimate fpr CM effects then composes these two, with weights proportional to the efficiency of each side.
...
\begin{figure}[h!]
    \caption{CF Adjusted Estimates of CF Effects Work with Different Error Term Parameters.}
    \begin{subfigure}[c]{0.475\textwidth}
        \centering
        \caption{ADE.}
        \includegraphics[width=\textwidth]{
