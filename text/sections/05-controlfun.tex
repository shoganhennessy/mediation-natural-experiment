...
Take $\hat \gamma$, the $D_i = 0$ subsample estimate of $\gamma$, and $(\hat{\gamma + \delta})$, the $D_i = 1$ subsample estimate of $(\gamma + \delta)$,
\[ \hat{\text{ADE}}^{\text{CF}}
    = (1 - \bar D) \, \hat \gamma + \bar D \, (\hat{\gamma + \delta}). \]

The AIE is less simple, for two reasons that differ from the parametric CF setting.
First, the the intercepts for each subsample, $\alpha$ and $(\alpha + \beta)$ are not separately identified from the CFs.
...
These problem can be avoided by estimating the AIE using its relation to the ATE.
Write $\hat{\text{ATE}}$ for the point-estimate of the ATE, to give the following representation,\footnote{
    \aref{appendix:semi-parametric} describes the reasoning for this estimator of the AIE in further detail.
}
\[ \hat{\text{AIE}}^{\text{CF}}
    = \hat{\text{ATE}} 
    - (1 - \bar Z) \, (\hat{\gamma + \delta})
    - \bar Z \, \hat \gamma. \]
