...
\begin{figure}[h!]
    ...
    $\vec U_i$ represents an unobserved confounding variable for the causal effect $D_i \to Y_i$, representing this paper's focus on the case that $D_i$ is not quasi-randomly assigned.
...
The group difference bias term arises because the convention approach to CM (selection-on-observables) assumes the complier average effect is equal to the population average effect, which does not hold true if the mediator is not quasi-randomly assigned.
