...
Now exploit a definition of the AIE,
\begin{align*}
    \text{AIE}
    &= \Prob{Z_i = 0}
    \underbrace{\Egiven{ Y_i(Z_i, D_i(1)) - Y_i(Z_i, D_i(0))}{ Z_i = 0}}_{
...
The semi-parametric AIE estimate then uses this representation, avoiding directly interacting with the estimated CFs, by plugging in estimates $\hat{\Pr}(Z_i =1)$, $\hat{\text{ATE}}$, and $\hat{\text{ADE}}$ conditional on $Z_i =0,1$, respectively.
