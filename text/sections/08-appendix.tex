...
Assume condition monotonicity (\ref{cf:monotonicity}) holds, for any treatment values $z < z'$ and covariate value $\vec X_i = \vec x$.
...
The result holds for continuous values of $Z_i$, so continue with the $\mathcal{Z}_0, \mathcal{Z}_1(i)$ notation.
...
Define:
\begin{align*}
    U_i &= F_V(V_i|\vec{X}_i)\\
    \pi(z'; \vec{X}_i) &= F_V(\psi(z'; \vec{X}_i)|\vec{X}_i) = \Probgiven{D_i = 1}{Z_i = z', \vec{X}_i}
\end{align*}
We can then equivalently represent the mediator choice as
...
\[ \Egiven{U_{0,i}}{\pi(Z_i; \vec X_i) < U_i} = \Egiven{\rho_0(F_V^{-1}(U_i) - \mu_V) + \varepsilon_{0,i}}{\pi(Z_i; \vec X_i) < U_i} \]
...
Therefore:
\[ \Egiven{U_{0,i}}{\pi(Z_i; \vec X_i) < U_i} = \rho_0 \Egiven{F_V^{-1}(U_i) - \mu_V}{\pi(Z_i; \vec X_i) < U_i}. \]
...
This relationship ensures consistency in the CF approach across both groups \citep{kline2019heckits}.
