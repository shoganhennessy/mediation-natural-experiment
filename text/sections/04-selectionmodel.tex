...
\[ D_i(z') = \indicator{ \psi \big( z'; \vec X_i \big) \geq V_i},
    \;\text{ for } z'=0,1 \]
where $V_i$ is a latent variable with continuous distribution and cumulative density function $F_V(.)$, and $\psi(.;\vec X_i)$ collects observed sources of mediator selection.
$V_i$ can be assumed to follow a known distribution; the canonical Heckman selection model assumes $V_i$ is normally distributed (a ``Heckit'' model).

I focus on the equivalent transformed model of \cite{heckman2005structural},
\[ D_i(z') = \indicator{ \pi(z'; \vec X_i) \geq U_i},
    \;\;\; \text{for } z'=0,1 \]
where $U_i \coloneqq F_V(V_i)$ follows a uniform distribution, and $\pi(z'; \vec X_i) = F_V\big(\psi(z'; \vec X_i)\big) = \Probgiven{D_i = 1}{Z_i = z', \vec X_i}$ is the mediator propensity score.
...
This assumption identifies the mediator propensity score separately for the direct and indirect effects, avoiding indeterminacy in the second-stage outcome equation.
