...
These estimands are typically estimated with linear models, and CM estimates are composed from OLS estimates \citep{imai2010identification}.
%\begin{align*}
%    D_i &= \phi + \pi Z_i
...
Rarely does research relying on a quasi-experimental research design employ an additional, overlapping identification design for $D_i$ as part of the analysis.
...
The group differences represent the fact that a matching estimator gives an average effect on the treated group and, when selection-on-observables does not hold, this is systematically different from the average effect \citep{heckman1998characterizing}.\footnote{
    ...
    These are referred to as bad controls by \cite{cinelli2024crash}, or M-bias by \cite{ding2015adjust}.
}$^{,}$\footnote{
