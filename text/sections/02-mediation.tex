...
\begin{enumerate}
    ...
    \item The Average Total Effect (ATE) refers to the effect of the treatment on outcome, $Z \to Y$, and is also known as the average treatment effect or intent-to-treat effect in social science settings, or reduced-form effect in the instrumental variables literature:
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}
        ...
        \node[state,blue] (treatment) [left=2.5cm of mediator] {$Z$};
        \node[state,red] (outcome) [right=2.5cm of mediator] {$Y$};
        % Label Z, D, Y
        \node[color=ForestGreen] [above=0.1cm of mediator] (mediator) {Mediator};
...
The selection bias terms come from systematic differences between groups take or refuse the mediator ($D_i = 0$ versus $D_i = 1$), differences not fully unexplained by $\vec X_i$.
