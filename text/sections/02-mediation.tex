...
To develop notation, write $Z_i =0,1$ for a binary treatment, $D_i=0,1$ a binary mediator, and $Y_i$ a continuous outcome for individuals $i = 1, \hdots, N$.
%\footnote{
...
\begin{enumerate}
    ...
    \[ \Egiven{D_i}{Z_i = 1} - \Egiven{D_i}{Z_i = 0}
        = \E{D_i(1) - D_i(0)}. \]
    It is common in the economics literature to assume that $Z$ influences $D$ in at most one direction, $\Prob{D_i(0) \leq D_i(1)} = 1$ --- monotonicity \citep{imbens1994identification}.
...
One might consider conventional CM methods in such a setting to learn about the mechanisms behind the causal effect $Z \to Y$.
...
It is important to acknowledge the mediator compliers here, because the AIE is the $Z \to Y$ effect going through the $D(Z)$ channel, thus only refers to individuals pushed into mediator $D$ by initial treatment $Z$.
If we had been using a population average effect for $D \to Y$, then this is losing focus on the definition of the AIE; it is not about the causal effect $D \to Y$, it is about the causal effect $D(Z) \to Y$.
