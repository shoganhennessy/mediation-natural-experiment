...
An instrumental variables approach assumes this direct effect is zero for everyone --- that health insurance increased health only via healthcare usage (i.e., the exclusion restriction).
...
\[ \text{AIE}
    = \E{Y_i(Z_i, D_i(1)) - Y_i(Z_i, D_i(0))}
    = \E{D_i(1) - D_i(0)} \; 
        \underbrace{\Egiven{Y_i(Z_i, 1) - Y_i(Z_i, 0)}{D_i(1) - D_i(0)}}_{
            \text{Average $D \to Y$ effect for compliers}
        } \]
