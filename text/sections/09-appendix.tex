...
\end{align*}
The proof is achieved by applying the expectation across $D_i = 0, 1$, and $\vec X_i$.
...
then applying the expectation across $Z_i = 0, 1$, and $\vec X_i$.
...
Thus, the first-stage regression of $Z \to Y$ has to unbiased estimates.
\begin{align*}
    D_i &= Z_i D_i(1) + (1 - Z_i) D_i(0) \\
        ...
        &= \underbrace{\E{D_i(0)}}_{\text{Intercept}} +
            ...
            & \;\;\;\; + \underbrace{D_i(0) - \E{D_i(0)}
                ...
                \text{Mean-zero independent error term, since }Z_i \indep D_i} \\
...
\end{align*}
These terms are conventionally estimated in a regression \citep{imai2010identification}.

\textbf{Senan note:} write here the $\Egiven{Y_i}{Z_i, D_i, \vec X_i}$ expression.
...
Rearrange the following to be
$\Egiven{Y_i}{Z_i =1, D_i} - \Egiven{Y_i}{Z_i =0, D_i}$ and such. 
\begin{align*}
...
\textbf{Senan note:} write here OLS estimates of CM estimands, with the OVB terms.
