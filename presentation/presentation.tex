    ...
    \begin{figure}[h!]
        ...
        \begin{subfigure}[c]{0.475\textwidth}
            ...
            \caption{People choosing healthcare
                $\to$ \textbf{SI} not.}
            \includegraphics[width=\textwidth]{presentation-files/headlines/health-differences.jpg}
        \end{subfigure}
    \end{figure}
\end{frame}
%-------------------------------------------------------------------------------
...
\begin{frame}[noframenumbering]
    ...
    \par\noindent\rule{\textwidth}{0.4pt}
    \textbf{Intuition:} Identify second-stage (despite correlated error term), to get ADE $+$ AIE.
\end{frame}
%-------------------------------------------------------------------------------
\begin{frame}[noframenumbering]
    \frametitle{Identification with a Selection Model}
...
\begin{frame}
    ...
    \end{align*}    
    Suppose each $i$ chooses to take \textcolor{ForestGreen}{mediator $D_i$} by a Roy model, with constant partial effects for $z', d' = 0, 1$.
    \begin{align*}
