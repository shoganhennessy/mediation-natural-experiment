...
\begin{frame}
    ...
    \end{figure}
    \textbf{Theorem:}
    If choice to attend healthcare is based on costs and benefits (Roy model) $\implies$ MI does not hold, unobserved confounding.
\end{frame}
%-------------------------------------------------------------------------------
\begin{frame}
    \frametitle{1. Selection Bias}
    In observational setting, must have an additional credible research design for \textbf{Mediator Ignorability (MI)} to hold.
    \vskip-0.5cm
    \begin{figure}[h!]
        \centering
        \singlespacing
...
\begin{frame}
    ...
    \vskip-0.5cm
    \par\noindent\rule{\textwidth}{0.4pt}
    If not, then CM effects are contaminated by bias terms.
...
\begin{frame}
    \frametitle{1. Selection Bias}
    In a simulation with Roy selection-into-$\textcolor{ForestGreen}{D_i}$, CM effects are biased.
...
\begin{frame}
    ...
    \begin{enumerate}
        ...
        \item There is an IV for mediator cost of take-up.
    \end{enumerate}
    ...
    \begin{figure}
        \begin{tikzpicture}
            ...
            \pause
            \node[align=left] [above left=-0.15cm and 0.1cm of mediator] {\textbf{1.} \textcolor{orange}{First-stage $> 0$}};
            \pause
    ...
    %\vfill
    %\textbf{Theorem:}
    %Average second-stage effects, $\textcolor{blue}{Z_i}, \textcolor{ForestGreen}{D_i} \to \textcolor{red}{Y_i}$ and thus the ADE $+$ AIE, are identified with corresponding Control Function (CF) adjustments.
\end{frame}
%-------------------------------------------------------------------------------
\begin{frame}
    \frametitle{2. CM with Selection --- Identification}
    \textbf{Theorem}:
    Under MTE assumptions, mediation second-stage effects,\\ $\textcolor{blue}{Z_i}, \textcolor{ForestGreen}{D_i} \to \textcolor{red}{Y_i}$, are identified by the MTE associated Control Functions (CFs).
    ...
    \vfill
    \textbf{Intuition:}
    Identifies ADE $+$ AIE by extrapolating from IV compliers to mediatior compliers (MTE extrapolation e.g., Mogstad Torgovitsky 2018).
\end{frame}
%-------------------------------------------------------------------------------
\begin{frame}
