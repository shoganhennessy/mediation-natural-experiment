...
\newcommand{\eqhighlight}[1]{\colorbox{yellow!75}{$\displaystyle#1$}}
\makeatletter
\let\HL\hl
\renewcommand\hl{%
    ...
    \begin{align*}
        D_i &= \indicator{
            Z_i(\delta + \beta) + (1 - Z_i)\beta
                \geq C_i - 
                \Big(\eqhighlight{U_{1,i} - U_{0,i}} \Big) } \\
        Y_i &= \alpha + \beta D_i + \gamma Z_i + \delta Z_i D_i
        + \zeta(\vec X_i)
        + \underbrace{\eqhighlight{
            \left(1 - D_i \right)U_{0,i} + D_i U_{1,i}}}_{
            \text{Correlated error term}}
    \end{align*}
    where $C_i$ are costs of taking $D_i$.
    \par\noindent\rule{\textwidth}{0.4pt}
    Selection model intuition $\implies$ use first-stage errors to purge second-stage correlated errors.
\end{frame}
%-------------------------------------------------------------------------------
\begin{frame}
    \frametitle{Identification with a Selection Model}
    ...
    \begin{itemize}
        ...
        \[ \text{Corr}\Big[ U_i, \left(1 - D_i \right)U_{0,i} + D_i U_{1,i} \Big] > 0. \]
...
\begin{frame}
    ...
    \begin{itemize}
        ...
        \item Use this approach to estimate direct and indirect effects of genetics and education (this is a companion paper)
        \item (eventually) R package for selection-adjusted CM effects, by Heckman model and IV-assisted CF/MTE.
    \end{itemize}
\end{frame}
%-------------------------------------------------------------------------------
\begin{frame}[noframenumbering]
