...
#! Test: do the WLS version of Abadie kappa weighting, to show why ATE est bad.
outcome = "Y_health"
Z = "any_insurance"
D = "any_healthcare"
...
Z_iv = "lottery_iv"
control_iv = "hh_size"
analysis.data$outcome    <- analysis.data[[outcome]]
analysis.data$Z          <- analysis.data[[Z]]
analysis.data$Z_iv       <- analysis.data[[Z_iv]]
analysis.data$control_iv <- analysis.data[[control_iv]]
analysis.data$X_minus <- analysis.data[[X_minus]]
est_probZ_iv <- glm(Z_iv ~ 1 + control_iv, data = analysis.data)
hat_probZ_iv <- est_probZ_iv$fitted
kappa.weight_0 <- (1 -analysis.data$Z) * (((1 -analysis.data$Z_iv
    ) - (1 - hat_probZ_iv)) / ( 
        (1 - hat_probZ_iv) * hat_probZ_iv))
kappa.weight_1 <- analysis.data$Z * ((analysis.data$Z_iv - hat_probZ_iv) / (
    (1 - hat_probZ_iv) * hat_probZ_iv))
analysis.data$kappa.weight <- kappa.weight_0 * (
    1 - hat_probZ_iv) + kappa.weight_1 * hat_probZ_iv

gam(outcome ~ 1 + Z + X_minus,
    weights = kappa.weight, data = analysis.data) %>% summary()

wls_negweights(outcome ~ 1 + Z,
    "kappa.weight",
    data = analysis.data) %>% summary()

reg.matrix <- cbind(analysis.data$Z, 1)
wls.est <- solve(t(reg.matrix * analysis.data$kappa.weight
    ) %*% reg.matrix) %*% t(reg.matrix * analysis.data$kappa.weight) %*% analysis.data$outcome
print(wls.est)

abadie.late(analysis.data, outcome, Z, Z_iv, control_iv)
# Attempt with WLS in LARF package.
library(LARF)

predict.larf <- function(object, newCov = NULL, newTreatment = NULL, ...) {
  if (is.null(newCov) & is.null(newTreatment)){
...
# Avoid the non-linear WLS approach, by a tiny transformation.
analysis.data <- analysis.data %>% drop_na()
unit.val <- 1 - 10^(-7)
analysis.data$any_healthcare <- unit.val * analysis.data$any_healthcare
analysis.data$Y_health <- unit.val * analysis.data$Y_health
analysis.data$Y_happy <- unit.val * analysis.data$Y_happy

heckit_firststage.reg <- larf(
    any_healthcare ~ 1 + hh_size,
    treatment = analysis.data$any_insurance,
    instrument = analysis.data$lottery_iv,
    method = "LS",
    data = analysis.data)
analysis.data$pi.est <- predict(heckit_firststage.reg)
larf(Y_happy ~ 1 + hh_size + any_healthcare,
    treatment = analysis.data$any_insurance,
    instrument = analysis.data$lottery_iv,
    method = "LS",
    data = analysis.data) %>% summary()

larf(Y_happy ~ 1 + hh_size,
    treatment = analysis.data$any_insurance,
    instrument = analysis.data$lottery_iv,
    method = "LS",
    data = analysis.data) %>% summary()
...
#TODO: solution, use a custom coded version of WLS, to use kappa weights.
#TODO: fuckkkkkkkk.
